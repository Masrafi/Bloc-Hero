...
class CorrigenTenderDetailsScreen extends StatelessWidget {
  const CorrigenTenderDetailsScreen({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (context) => CorrigenTenderDetailsBloc(
            RepositoryProvider.of<RepositoryCorrigenTenderDetails>(context),
          )..add(
              CorrigenTenderDetailsLoadEvent(),
            ),
        ),
        BlocProvider(
      ...
      child: Scaffold(
          appBar: header(context, titleText: "Corrigendum"),
          body: BlocListener<SaveTenderBloc, SaveTenderState>(
            ...
            child: BlocBuilder<CorrigenTenderDetailsBloc,
                CorrigenTenderDetailsState>(builder: (context, state) {
              if (state is CorrigenTenderDetailsLoadingState) {
                return const Center(
                  child: CircularProgressIndicator(),
                );
              }
              if (state is CorrigenTenderDetailsErrorState) {
                return Center(
                  child: Text("Error"),
                );
              }
              if (state is CorrigenTenderDetailsLoadedState) {
                List<CorrigenTenderDetailsModel> dataList = state.users;
